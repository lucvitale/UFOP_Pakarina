let currentClimateRequest = null;

function getClimateTranslation(key, fallback) {
...
function analyzeClimate(data) {
  ...
  if (riskScore >= 5) {
    ...
    riskMessage =
      t[riskMsgKey] ||
      "⚠️ Current conditions are highly favorable for dengue mosquito proliferation. Take preventive measures.";
    ...
    riskMessage =
      t[riskMsgKey] ||
      "⚡ Conditions partially favor mosquito activity. Stay alert and eliminate standing water.";
  } else {
    ...
    riskMessage =
      t[riskMsgKey] ||
      "✅ Current conditions are not particularly favorable for dengue mosquito proliferation.";
  }

  const cityNameElement =
    document.getElementById("risk-city-name");
  ...
  document.getElementById("ind-precipitation").textContent =
    `${precipitation} mm`;

  const tempPct = Math.min(
    Math.max((temp / 45) * 100, 0),
    100
  );

  const humPct = Math.min(humidity, 100);
  const windPct = Math.min((wind / 20) * 100, 100);
  const precipPct = Math.min(
  ...
  setBar(
    "bar-temp",
    tempPct,
    temp >= 25 && temp <= 35 ? riskColor : "#a48a6a"
  );

  setBar(
    "bar-humidity",
    humPct,
    humidity >= 60 ? riskColor : "#a48a6a"
  );

  setBar(
    "bar-wind",
    100 - windPct,
    wind < 3 ? riskColor : "#a48a6a"
  );

  setBar(
    "bar-precipitation",
    precipPct,
    precipitation > 5 ? riskColor : "#a48a6a"
  );
  ...
  setHint(
    "hint-precipitation",
    getPrecipMsgKey(precipitation),
    "Rain increases mosquito breeding sites by creating standing water."
  );

  document.getElementById("climate-loading").style.display =
    "none";

  document.getElementById("climate-no-data").style.display =
    "none";

  document.getElementById("climate-results").style.display =
    "block";
}
...
function getHumidityMsgKey(humidity) {
  if (humidity < 40) {
    return "climate_humidity_msg_low";
  }

  if (humidity < 60) {
    return "climate_humidity_msg_medium";
  }
...
function getWindMsgKey(wind) {
  return wind < 3
    ? "climate_wind_msg_low"
    : "climate_wind_msg_high";
}

function getPrecipMsgKey(precipitation) {
  if (precipitation <= 5) {
    return "climate_precip_msg_low";
  }

  if (precipitation <= 20) {
    return "climate_precip_msg_medium";
  }
...
function setHint(id, key, fallback) {
  ...
  el.textContent = getClimateTranslation(key, fallback);
...
function showClimateError() {
  ...
  const messageElement = document
    .getElementById("climate-no-data")
    .querySelector("p");
  ...
  messageElement.textContent = getClimateTranslation(
    "climate_error",
...
function analyzeCity() {
  const input = document
    .getElementById("climate-city-input")
    .value
    .trim();

  if (input) {
    fetchClimateData(input);
  }
}
...
document.addEventListener("languageChanged", () => {
  if (!currentClimateRequest) return;

  if (currentClimateRequest.type === "city") {
    fetchClimateData(currentClimateRequest.value);
  }

  if (currentClimateRequest.type === "coords") {
    fetchClimateByCoords(
      currentClimateRequest.lat,
      currentClimateRequest.lon
    );
  }
});
...
const climateCityInput = document.getElementById(
  "climate-city-input"
);

if (climateCityInput) {
  climateCityInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      analyzeCity();
    }
  });
